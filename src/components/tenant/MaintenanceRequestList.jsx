...
const MaintenanceRequestList = () => {
  ...
  if (loading) {
    return <div className="maintenance-list-container">Loading maintenance requests...</div>;
  }

  if (error) {
    return <div className="maintenance-list-container error-message">Error: {error}</div>;
  }
  ...
  return (
    <div className="maintenance-list-container">
      ...
      {filteredRequests.length === 0 ? (
        <p>No maintenance requests found {currentUser && currentUser.role === 'tenant' ? 'for your unit.' : '.'}</p>
      ) : (
